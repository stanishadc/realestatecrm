...
const PassbookDocuments = () => {
  ...
  return (
    <div style = {{marginTop: "55px"}}>
      ...
      <table className='styled-table'>
        <thead>
          <tr>
            ...
            <th style = {{textAlign: "center"}}>PassbookDocumentsId</th>
        ...
        <tbody>
          {data.map((item,index)=>{
            return(
              <tr key = {item.id}>
                <td>{item.DummyId}</td>
                <td>{item.PassbookDocumentsId}</td>
      ...
      <Link to='/passbookdocuments'>PassbookDocuments</Link><br/>
      <Link to='/'>Home</Link>
    </div>
  );
};

export default PassbookDocuments;